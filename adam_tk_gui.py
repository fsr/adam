...
class ReportGenForm(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        ttk.Button(self, width=0, text="Create PDF", command=self.create_pdf).grid(row=8, column=0, columnspan=3,
                                                                                   sticky="EW")
    ...
    def create_pdf(self):
        filename = asksaveasfilename(title="Save as PDF (additional .json file will be created)",
                                     filetypes=[("PDF", "*.pdf")])
        if not filename == "":
            # Create report first...
            ...
            report = construct_report.construct_report(reportdef, questionsdef, answersdef, self.dbfilename,
                                                       self.coursevar.get())
